...
export const CONNECTABLE_SIGNAL_SOURCE_IDS = ['GITHUB', 'HACKERRANK', 'LEETCODE'] as const;
...
export const ConnectGithubSignalRequestSchema = z.object({
  ...
  selectedRepoFullNames: z.array(z.string().min(1).max(200)).max(5).optional(),
...
export const RefreshSignalsRequestSchema = z.object({
  sourceIds: z.array(ConnectableSignalSourceIdSchema).max(3).optional(),
});
