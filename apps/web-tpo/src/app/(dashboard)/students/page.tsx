...
export default function TpoStudentsPage() {
  ...
  return (
    <main className="max-w-[1400px] mx-auto space-y-6 font-sans select-none">
      ...
      <div className="bg-white p-6 rounded-md border border-slate-200/80 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5 mb-1">
            <h1 className="text-xl font-bold tracking-tight text-slate-900">Candidate Roster</h1>
            <span className="bg-[#004c63]/10 text-[#004c63] border border-[#004c63]/20 text-[11px] font-bold px-3 py-0.5 rounded-full">
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="primary"
            className="bg-[#004c63] hover:bg-[#003a4d] text-white flex items-center gap-2 font-bold shadow-sm px-4 py-2.5 rounded-md text-xs"
      ...
      {/* Alert Banners */}
      {error && (
        <div className="bg-rose-50 border border-rose-200 text-rose-800 p-4 rounded-md flex items-center justify-between shadow-xs animate-in fade-in duration-200">
          <div className="flex items-center gap-3">
      ...
      {message && (
        <div className="bg-emerald-50 border border-emerald-200/90 text-emerald-900 p-3.5 rounded-md flex items-center justify-between shadow-xs animate-in fade-in duration-200">
      ...
      {/* Filters Toolbar */}
      <Card className="bg-white border border-slate-200/80 shadow-sm p-4 flex flex-col md:flex-row gap-3 rounded-md">
        <div className="relative flex-1">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
          <input
            type="text"
            placeholder="Search candidates by name or email..."
            className="w-full bg-white text-slate-900 text-xs rounded-md py-2.5 pl-10 pr-4 border border-slate-200/90 focus:outline-none focus:border-[#004c63] focus:ring-2 focus:ring-[#004c63]/15 transition-all placeholder:text-slate-400 font-medium shadow-xs"
            value={q}
        ...
        <div className="flex gap-3">
          <select
            className="bg-white text-slate-700 text-xs rounded-md border border-slate-200/90 px-4 py-2.5 focus:outline-none focus:border-[#004c63] font-semibold cursor-pointer shadow-xs"
            value={inviteStatus}
          ...
          <Button
            type="button"
            className="bg-[#004c63] hover:bg-[#003a4d] text-white flex items-center gap-2 font-bold text-xs rounded-md px-4 py-2.5 shadow-xs"
            onClick={() => {
              setError(null);
              setMessage(null);
              load().catch((err) =>
      ...
      {/* Data Table */}
      <Card className="bg-white border border-slate-200/80 shadow-sm overflow-hidden rounded-md">
        {students.length === 0 ? (
          <div className="p-12 flex flex-col items-center justify-center text-center">
            ...
            <Button
              variant="outline"
              className="border-slate-200 text-slate-700 hover:bg-slate-50 rounded-md text-xs font-semibold"
              onClick={() => {
                setQ('');
                setInviteStatus('');
                load();
                ...
                {students.map((student) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-slate-600">
                      <span className="bg-slate-100 border border-slate-200/90 text-slate-700 px-2.5 py-1 rounded-md text-xs font-semibold">
                    ...
                    {/* Verification */}
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-sky-50 text-sky-700 border border-sky-200/90">
                      ...
                      >
                        {student.inviteStatus === 'PENDING' && (
                          <button
                            ...
                            className="bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 px-3 py-1.5 rounded-md text-xs font-bold flex items-center gap-1.5 shadow-xs transition-all disabled:opacity-50"
                        ...
                        {student.heldAt ? (
                          <button
                            className="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200/90 px-3 py-1.5 rounded-md text-xs font-bold flex items-center gap-1.5 shadow-xs transition-all"
                            onClick={async () => {
                              try {
                          ...
                          <button
                            className="bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200/90 px-3 py-1.5 rounded-md text-xs font-bold flex items-center gap-1.5 shadow-xs transition-all"
                            onClick={async () => {
                              try {
