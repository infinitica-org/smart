  ...
  return (
    <div className={candidatesPageStackClass}>
      <TpoBentoPageHeader
        ...
        description="Onboard candidates by email. Candidates autonomously select their own streams during onboarding."
        icon={UserPlus}
        accent="mint"
        badge={
      ...
      <div className={bentoCompactToolbarClass}>
        ...
        {scaffoldLoading ? (
          ...
          <p className="text-[13px] text-[var(--ds-text-muted)]">
            ...
            <a href="/batches" className="font-semibold text-[var(--ds-link)] underline">
              Batches
            </a>{' '}
            in the sidebar, then return here to onboard candidates.
          </p>
        ) : (
          <select
      ...
      <div className={bentoCompactCardClass}>
        <div className={bentoSegmentedTabsClass}>
          {(
            [
              ...
              ['bulk', 'Bulk Email Paste', ListPlus],
              ['csv', 'CSV Roster Upload', FileSpreadsheet],
            ] as const
          ).map(([tab, label, Icon]) => (
            <button
              key={tab}
        ...
        {activeTab === 'csv' && (
          <div className="max-w-3xl space-y-3">
            <div className="flex flex-col items-center justify-center rounded-[14px] border-2 border-dashed border-[var(--ds-border)] bg-[var(--ds-surface-muted)] p-5 text-center transition-colors hover:border-[#dfe4ec]">
              <UploadCloud className="mb-2 size-8 text-[var(--ds-text-muted)]" />
              <p className="mb-1 text-sm font-semibold text-[var(--ds-text)]">
                Upload Candidate CSV Roster
              </p>
              <p className={`${dashboardMetricHintClass} mb-4 text-[12px]`}>
