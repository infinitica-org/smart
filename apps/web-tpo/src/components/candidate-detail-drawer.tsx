...
export function CandidateDetailDrawer({
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-2xl bg-white border-l border-slate-200 shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col font-sans">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-slate-200 bg-slate-50/80">
          ...
          <button
            onClick={onClose}
            className="p-2 text-slate-400 hover:text-slate-600 transition-colors bg-white border border-slate-200 rounded-full shadow-sm"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-full bg-[#004c63]/10 border border-[#004c63]/20 flex items-center justify-center text-[#004c63] font-bold text-xl shrink-0">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-3.5">
              <p className="text-xs text-slate-500 font-medium mb-1">Batch</p>
              <p className="text-slate-900 font-semibold">{candidate.batchName ?? 'No batch'}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-3.5">
              <p className="text-xs text-slate-500 font-medium mb-1">Invite status</p>
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-3.5">
              <p className="text-xs text-slate-500 font-medium mb-1">Accepted</p>
              <p className="text-slate-900 font-semibold">
              ...
              <ul className="space-y-2">
                {claims.map((claim) => (
                  ...
                  >
                    ...
                    <span
                      className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-bold ${statusTone(claim.status)}`}
                    >
