...
| Merging                 | All PRs into `dev` are reviewed and merged exclusively by Tino (`@brittytino`). No module-owner-only merges. Promotion (`dev → qa → main`) is done by Tino only.                                                   |
